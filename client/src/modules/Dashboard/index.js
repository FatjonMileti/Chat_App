...
const Dashboard = () => {
    ...
    socket?.on("getMessage", (data) => {
      ...
      setMessages((prev) => ({
        ...
        messages: prev.messages
          ? [...prev.messages, { user: data.user, message: data.message }]
          : null,
      }));
      ...
      if (data.senderId !== user?.id && notificationRef.current) {
        toast.success(`New message from ${data.user.fullName}`, {
          position: "top-left",
        });
      }
      ...
      socket.on("getNotification", (d) => {
        if (
          ...
          setNotifications((prev) => [
            ...
            {
              [d.senderName]:
                prev.filter((notification) => notification[d.senderName])
                  .length + 1,
            },
          ]);
          ...
          /*setNotifications((prev) => ({
            ...prev,
            [d.senderName]: (prev[d.senderName] || 0) + 1,
          }));*/
        }
      });
    });
  ...
  const fetchMessages = async (conversationId, receiver) => {
    ...
    notificationRef.current = false;
    setNotifications(notifications.map((n) => (n[receiver.fullName] = 0)));
  };
